...
public class MazeDFSGeneratorTest {
	...
	public void shouldReturnUnvisitedNeighbour(int width, int height, int x, int y, int vx, int vy, int rand, Direction answer){
		...
		Direction unvisitedNeighbour = generator.getRandomUnvisitedNeighbour(x, y);
	...
	public void shouldBeUnableToGetUnvisitedNeighbour(){
		...
		Direction unvisitedNeighbour = generator.getRandomUnvisitedNeighbour(7,5);
	...
	public void shouldAskForRandomNeighbourOfCell(int width, int height, int x, int y){
		...
		generator.nextStep();
		verify(stack).empty();
		verify(generator).getRandomUnvisitedNeighbour(x, y);
	}
	...
	public void shouldNotAskForRandomNeighbourOfCell(int width, int height, int x, int y){
		...
		generator.nextStep();
		verify(stack).empty();
		verify(generator, never()).getRandomUnvisitedNeighbour(anyInt(), anyInt());
	}
	...
	public void shoulAskForNeighbourCoordinates(int width, int height, int x, int y, int nx, int ny, Direction direction){
		...
		Maze maze = spy(new Maze(width,height));
		generator.setMaze(maze);
		doReturn(direction).when(generator).getRandomUnvisitedNeighbour(x, y);
		generator.getRouteCoordinatesStack().push(new Coordinates2D(x,y));
		generator.nextStep();
	...
	public void shouldNotAskForNeighbourCoordinates(int width, int height, int x, int y, int nx, int ny, Direction direction){
		...
		Maze maze = spy(new Maze(width,height));
		generator.setMaze(maze);
		doReturn(null).when(generator).getRandomUnvisitedNeighbour(x, y);
		generator.getRouteCoordinatesStack().push(new Coordinates2D(x,y));
		generator.nextStep();
	...
	@SuppressWarnings("unused")
	private static final Object[] getSizesCoordinatesRandsAndDirection(){
		return new Object[]{
				new Object[]{8,8,0,5,3,5,Direction.WEST},
				new Object[]{3,3,2,1,1,1,Direction.EAST},
				new Object[]{12,12,6,0,0,6,Direction.NORTH},
				new Object[]{5,9,3,8,2,3,Direction.SOUTH},
				new Object[]{17,19,0,5,3,5,Direction.WEST},
				new Object[]{22,23,21,1,1,1,Direction.EAST},
				new Object[]{15,16,6,0,0,6,Direction.NORTH},
				new Object[]{14,18,3,17,2,3,Direction.SOUTH}
		};
	}
	
	@Test
	@Parameters(method = "getSizesCoordinatesRandsAndDirection")
	public void shouldRemoveWallAndReturnCoordinates(int width, int height, int x, int y, int rnd, int rnd2, Direction direction){
		MazeDFSGenerator generator = spy(new MazeDFSGenerator(width, height));
		Maze maze = spy(new Maze(width,height));
		Random rng = mock(Random.class);
		when(rng.nextInt(4)).thenReturn(rnd);
		when(rng.nextInt(height)).thenReturn(rnd2);
	...
	@Test
	@Parameters(method = "getSizesWithTwoCellsCoordinates")
	public void shouldSetStartAndFinishCoordinates(int width, int height, int sx, int sy, int fx, int fy){
		MazeDFSGenerator generator = spy(new MazeDFSGenerator(width, height));
		Maze maze = spy(new Maze(width,height));
		when(generator.getNumberOfUnvisitedCells()).thenReturn(0);
